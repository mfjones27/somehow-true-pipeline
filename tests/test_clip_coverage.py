...
class FillDurationsTest(unittest.TestCase):
    ...
    def test_rounds_up_to_runway_minimum(self):
        self.assertEqual(fill_clip_durations(0.4), [2])
        self.assertEqual(fill_clip_durations(1), [2])
    ...
    def test_splits_beyond_max(self):
        self.assertEqual(fill_clip_durations(11), [10, 2])
        self.assertEqual(fill_clip_durations(20), [10, 10])
        self.assertEqual(fill_clip_durations(21), [10, 10, 2])
...
class PlanCoverageTest(unittest.TestCase):
    ...
    def test_fct023_grows_existing_scenes_instead_of_a_seventh(self):
        ...
        self.assertEqual(durations, [10, 10, 9, 10, 10, 10])

    def test_adds_fill_clip_when_six_tens_are_not_enough(self):
        prompts = [f"scene {i}" for i in range(6)]
        planned, durations = plan_scene_coverage(prompts, [10] * 6, 66.2)
        self.assertEqual(durations[:6], [10] * 6)
    ...
    def test_hold_prompt_stays_within_runway_limit(self):
        text = hold_prompt("word " * 400)
        self.assertLessEqual(len(text), 950)
